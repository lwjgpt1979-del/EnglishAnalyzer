...
import { seHwBatches, seCourseUnits,
...
// 点作业名 → 跳独立卷详情页(sentence-paper),原生返回回本列表
function openById(id: string) {
  const g = groups.value.find(x => x.id === id)
  if (!g) return
  const t = encodeURIComponent(g.title || '长难句精讲')
  const s = encodeURIComponent(g.sub || '')
  uni.navigateTo({ url: `/pages/intensive/sentence-paper?mode=homework&id=${id}&title=${t}&sub=${s}` })
}
...
// 点单元 → 跳独立卷详情页(sentence-paper),原生返回回来
function openCourseUnit(unitId: string) {
  const u = courseUnits.value.find(x => x.unit_id === unitId)
  if (!u) return
  const t = encodeURIComponent(u.unit_title || '长难句精讲')
  const s = encodeURIComponent(`第${u.unit_no}单元`)
  uni.navigateTo({ url: `/pages/intensive/sentence-paper?mode=course&id=${unitId}&title=${t}&sub=${s}` })
}
