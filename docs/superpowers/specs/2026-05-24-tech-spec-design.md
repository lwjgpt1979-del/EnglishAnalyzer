...
| Token 类型 | 有效期 | 机制 |
|-----------|--------|------|
| access_token | **2 小时** | 每次 API 请求携带；过期后由客户端静默刷新 |
| refresh_token | **30 天** | 存储于客户端安全区；用于换取新 access_token |
| 强制重新登录 | refresh_token 过期时 | 重新走微信 wx.login → OAuth 流程 |

- 微信 `session_key` 不透传业务层（见 §1.2），仅在 Auth 层用于 wx.login 兑换
- refresh_token 失效场景：主动登出、30 天未使用、账号被封禁
...
机构城市（institution）
    ...
    > 平台手动修正（manual，超管显式干预；可覆盖 self_selected，不可覆盖 institution / cert_verified）
