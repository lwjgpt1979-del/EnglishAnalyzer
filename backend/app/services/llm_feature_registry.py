...
LLM_FEATURES: list[dict] = [
    ...
    {"feature": "reading_qtype_classify", "mode": "chat", "surface": "运营后台", "module": "作业·阅读题型归类(学情统计P1)", "service": "reading_qtype_service",
     "purpose": "把未精讲的作业阅读小题归类到固定题型(细节/主旨/推理/词义/态度/指代/图表/其他)", "why": "规格明确的单标签分类,快档即可,不占推理档;仅补跑精讲未覆盖的题", "locations": ["reading_qtype_service.py:classify_missing"], "cache": "global", "store": "user_paper_questions.reading_skill(按题内容 md5 去重,同内容只调一次;回填优先)"},
