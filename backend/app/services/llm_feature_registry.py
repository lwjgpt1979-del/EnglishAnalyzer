...
LLM_FEATURES: list[dict] = [
    ...
    {"feature": "vocab_kp_review", "mode": "reasoning", "surface": "运营后台", "module": "词力通·考点AI自审校", "service": "word_kp_service",
     "purpose": "低峰批量审校未审校词的『用法/考法类文本维』考点(及物性/语态/句型/可数性/所有格/-ed-ing/介词辨析/用法/语义侧重/考法):删明显错项、改表述不准", "why": "逐条判断考点正确性/义项归属→多步推理→推理档;可链维已 morph/wordnet/词库背书、搭配已语料印证不重审;低峰调用省钱", "locations": ["word_kp_service.py:_review_kp_rows"], "cache": "none", "store": "改/删 vocab_word_relation + 置 vocab_word_kp.reviewed_at + 记 vocab_word_kp_review(before/after)"},
