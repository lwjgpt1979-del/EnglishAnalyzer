...
并带 exchange(词形变换码)供考点形态学打底。**每环境各自导入一次,不进 content_seed**
(免把 63MB 塞进生产种子)。表结构由迁移 m200 + m203(exchange) 建;本脚本只灌数据
(幂等:TRUNCATE 后重灌)。升级 exchange 列后须重跑本脚本一次。
...
async def main() -> None:
    ...
    async with _async_session_factory() as s:
        ...
        ap = raw.driver_connection            # psycopg.AsyncConnection(COPY 用)
        await ap.execute("""
            CREATE TABLE IF NOT EXISTS dict_ecdict (
                ...
                translation TEXT, tag VARCHAR(64), exchange VARCHAR(256));
            CREATE INDEX IF NOT EXISTS ix_dict_ecdict_lower ON dict_ecdict (word_lower);
        """)
        await ap.execute("ALTER TABLE dict_ecdict ADD COLUMN IF NOT EXISTS exchange VARCHAR(256)")
        ...
        cols = ",".join(_STG_COLS)
        copy_sql = f"COPY stg({cols}) FROM STDIN WITH (FORMAT csv, HEADER true)"
        async with ap.cursor() as cur:
        ...
        await ap.execute("""
            INSERT INTO dict_ecdict (word, word_lower, phonetic, translation, tag, exchange)
            SELECT word, lower(word), NULLIF(phonetic,''), NULLIF(translation,''),
                   NULLIF(tag,''), NULLIF(exchange,'')
            FROM stg WHERE translation IS NOT NULL AND translation<>''""")
        cur = await ap.execute("SELECT count(*) FROM dict_ecdict")
        n = (await cur.fetchone())[0]
        cur = await ap.execute(
            "SELECT count(*) FROM dict_ecdict WHERE exchange IS NOT NULL AND exchange<>''")
        n_ex = (await cur.fetchone())[0]
        print(f"✓ dict_ecdict 导入完成:{n} 行(含 exchange {n_ex})")
        await s.commit()
