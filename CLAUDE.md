...
**凡「系统出题 / 生成题 / 仿真练习」给学生前端作答的题目——练同类、作业练同类、语法/词汇精讲练习、自适应练习、词汇仿真题等——一律只出「单选选择题」(题干 + ≥2 个选项 + 唯一正确项);严禁产出填空、翻译、简答、作文等无法客观即时判分的题型。** 因为逐题作答统一走 `PracticeQuiz`(点选项即时判分),只有选择题能无歧义自动判分;非选择题混进来会判不了分、坏交互。**注意:这条约束的是「系统生成的练习题」,不是学生原始错题本身**(原错题可能是填空/完形,按原样展示复习)。

铁律:
- **生成端只出单选**:后端出题一律 `question_type="单选"`,`content = {stem, options:[≥2], answer, explanation}`,`answer` 必须与某个 option 完全一致;Prompt 明确「每题 4 个选项、单选」。参考 `practice_service.generate_practice_questions`(义→词/挖空/词→义等全拼成选择题)、`wrong_center_service` 词汇仿真题。
- **缺项即丢弃、不硬塞**:生成/校验时丢掉 `options` 为空或不足 2 项、`answer` 不在 options 里的题(`practice_service` 已有校验);宁可少出几题,也不返回非选择题。返回前可再过滤一层保险。
- **前端只吃选择题结构**:`PracticeQuiz` 的 `questions` 只认 `{id, stem, options, answer, explanation}`;拿不到合法 `options` 的题不进作答流。
- **例外(非「系统出题作答」,不受此约束)**:①**BKT 探针**(词力通接收/产出、长难句释义·短翻译·迁移、语法探针)——本就是非选择题的能力检测,单独设计;②**机构作业答题**(学生自由作答、提交老师判分);③**学生原始错题 / 真题原题**的原样展示(题型按原卷)。
- **新接「系统出题」能力**一律照此:先想清楚怎么把考点拼成单选(正确项 + 干扰项),不允许生成主观题让学生前端答。

## 出题句子用词要简单:别让载体词干扰目标考点(全项目强制)

**凡为「学某个词 / 某个考点」而 LLM 生成的选择题(词汇精讲、考点扩展测试、练同类等),题干句 / 例句里除「目标词 + 必要的考点词(选项里那些词)」外,其余用词一律要简单——不高于目标词的难度;严禁用比目标词更生僻的词做句子载体。** 否则学生卡在句子里不认识的其它难词上,根本落不到要考的那个考点/词,既学不到、又判分失真(典型事故:考 `celebrate` 的搭配,句子里塞进 `predominantly`、`albeit` 这类更难的词,学生先被难词绊住)。

铁律:
- **载体词服从目标词**:句子只是承载考点的语境,难度由目标词封顶。目标词 + 考点词(近义/反义/派生/易混/搭配里出现的词)可用,其余实词尽量用初中级高频词;不确定就用更简单的说法。
- **写进 Prompt**:所有此类出题的 system prompt 必须明确「题干句除目标词与考点词外,其余单词要简单常见(不高于目标词难度),不要用更生僻的词」。参考 `word_kp_service._gen_kp_mcqs`、`vocab_mcq_service._gen_mcqs`、`practice_service.generate_practice_questions`。
