...
- **按内容单元分级缓存(全部+部分)**:同一份内容常被**部分/多次上传**(同一份作业分几次拍、每次只拍部分题型;不同学生传同一份卷;同卷重拍不同照片)。缓存粒度要贴合「**可复用的最小内容单元**」,让部分重叠也能命中、不重复付费——按内容判定单元并各自 md5:整份(全文)/ 题型块(阅读·完形按短文)/ 小题(独立题按题干、关联题带短文上下文)/ 句(长难句)/ 词 / 图。参考实现:`ocr_cache`(图 md5,OCR/视觉一步出结果)、`kp_classify_cache`(小题 md5,归类)、`sentence_analysis_cache`(句 md5)、`vocabulary_words`(词)、`user_uploaded_papers.content_hash`(整卷重拍去重、指向原卷)。**新接「上传→解析」类能力**先想清楚「内容单元怎么切、每级按什么 md5 缓存」,别只做整份级、漏掉部分复用。
- **视觉/OCR 一步出结构**(如豆包 Vision 直接返回结构化 JSON):此时「OCR + 拆题」是同一次付费调用,按**图 md5** 缓存即覆盖两者,不存在可再单独缓存的「拆题 LLM」——别为已合并的步骤重复造缓存。
